...
extract.frames <- function(fps, inpath=NULL, outpath="frames", filetypes=c(".mp4", ".avi"),
  ...
  extract.dir <- function(inpath){
    ...
    if(any(isvid)){
      ...
      if(stamp.time==TRUE)
        stamptime(fps, outpath, exf, epath)
      if(copy.jpegs==TRUE) 
...
stamptime <- function(fps, path, vexf, toolpath="C:/Exiftool"){
  wd <- getwd()
  setwd(toolpath)

  ffls <- list.files.only(path, full.names=TRUE)
  ffls <- sub("\\(", "\\\\(", ffls)
  ffls <- sub("\\)", "\\\\)", ffls)
  fls <- basename(ffls)
  ...
  vcd <- vexf$CreateDate[i]
  exf$FrameNumber <- nums
  exf$VideoCreateDate <- ifelse(is.na(nums), NA, vcd)
  exf$FrameExtractRate <- ifelse(is.na(nums), NA, fps)
  exf$CreateTimeOffset <- nums/fps
  cdate <- strptime(exf$VideoCreateDate, "%Y:%m:%d %H:%M:%S", tz="UTC")+exf$CreateTimeOffset
  exf$CreateDate <- ifelse(is.na(nums), vcd, strftime(cdate, "%Y:%m:%d %H:%M:%S"))
  mfile <- paste(path, "metadata.csv", sep="/")
  write.csv(exf, mfile, row.names=FALSE)
  cmd <- paste0('exiftool -csv="', mfile, '" ', path, " -overwrite_original")
  shell(cmd)
  file.remove(mfile)
