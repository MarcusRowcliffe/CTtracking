...
```{r echo=FALSE, fig.cap="Fig. 1. Example image of a camera calibration pole in position 2 m from the camera.", out.width="60%", fig.align="center"}
...
3.	Repeat this for further pole placements across the field of view and away from the camera with placements spaced about 1 m apart. Continue away from the camera to the maximum extent that any animals are likely to be captured, or if possible a bit beyond. As you reach greater distances, it may help to have second person next to the camera to keep it triggering. Aim for 10-20 placements, with more if visibility is greater. Fig. 4 shows an ideal configuration of deployment calibration pole placements.

```{r echo=FALSE, fig.cap="Fig. 3. Example image of a deployment calibration pole placement.", out.width="60%", fig.align="center"}
pth <- "C:/Users/rowcliffe.m/OneDrive - Zoological Society of London/GitHub/CTtracking/Survey_yyy/DeploymentImages/01/IMG_0096.JPG"
knitr::include_graphics(pth)
```

```{r echo=FALSE, fig.cap="Fig. 4. Diagram of an example set of pole locations for a deployment calibration.", out.width="60%", fig.align="center"}
th <- c(70, 90, 110)*pi/180
rr <- seq(0.5, 3.5, 0.5)
dat <- expand.grid(rr,th)
thjit <- 0.06
...
6. To start a new image directory, refresh your browser and repeat from step 2.
...
3. At least the first position of every individual animal sequence should be digitised, but it is not necessary to digitise every subsequent image if the animal becomes increasingly distant and/or hard to locate with certainty.
...
```{r echo=FALSE, fig.cap="Fig. 7. Animaltracker screenshot, showing the first image of a sequence with the animal only partially visible, and the subsequently digitised track showing.", out.width="100%", fig.align="center"}
...
```{r echo=FALSE, fig.cap="Fig. 5. Diagnostic plots for a camera calibration model", fig.align="center"}
par(mfrow=c(1,2))
plot(cmods[[1]])
```
...
plot(smods)
```
```{r echo=FALSE, fig.cap="Fig. 6. Diagnostic plots for a deployment calibration model", fig.align="center"}
par(mfrow=c(1,2))
plot(smods$'01')
```
...
```
It is likely that radial distances for some records will have been poorly estimated where animals move beyond the deployment calibration grid, so it is worth exploring the distribution, perhaps going back to the digitisations to check for problems (e.g. substantial shift in camera orientation during the deployment), and judiciously removing any remaining extreme observations from subsequent analyses.
